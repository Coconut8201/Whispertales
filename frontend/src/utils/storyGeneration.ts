...
import { PicStyle, picStyleList } from "./bookStyleList";
 ...
 * 格式化角色關係
 */
export const formatRelationships = (
  relationships: CharacterRelationship[],
...
export const getStyleOptions = (searchQuery: string): string[] => {
  ...
  return [searchQuery, ...options.map((option) => option.show_name)];
};

/**
 * 根據風格名稱找到對應的模型
 */
export const findModelByStyleName = (
  styleName: string,
): PicStyle | undefined => {
  return picStyleList.find((model) => model.show_name === styleName);
};
...
export const generateStory = async (
  ...
  try {
    ...
    const data = {
      ...
      otherCharacters: formData.otherCharacters.filter(
        (character) => character !== "",
      ),
