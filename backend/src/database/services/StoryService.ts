...
export class StoryService {
   ...
   * @returns 操作結果
   */
  static async addToFavorite(storyId: string): Promise<OperationResult> {
    try {
      const book = await storyModel.findById(storyId);

      if (!book) {
        console.error(`[StoryService.addToFavorite] 找不到故事: ${storyId}`);
        return {
          success: false,
          message: "找不到此故事",
        };
      ...
      console.log(`[StoryService.addToFavorite] 成功添加故事 ${storyId} 到我的最愛`);
      return {
        success: true,
        message: "成功添加到我的最愛",
      };
    } catch (e: any) {
      console.error(`[StoryService.addToFavorite] 添加到我的最愛失敗: ${e.message}`);
      return {
        success: false,
        message: `添加到我的最愛失敗: ${e.message}`,
      };
    }
  }
   ...
   * @returns 操作結果
   */
  static async removeFromFavorite(storyId: string): Promise<OperationResult> {
    try {
      const book = await storyModel.findById(storyId);

      if (!book) {
        console.error(
          `[StoryService.removeFromFavorite] 找不到故事: ${storyId}`,
        );
        return {
          success: false,
          message: "找不到此故事",
      ...
      console.log(
        `[StoryService.removeFromFavorite] 成功從我的最愛移除故事 ${storyId}`,
      );
      return {
        success: true,
        message: "成功從我的最愛移除",
      ...
      console.error(
        `[StoryService.removeFromFavorite] 從我的最愛移除失敗: ${e.message}`,
      );
      return {
        success: false,
        message: `從我的最愛移除失敗: ${e.message}`,
      };
    }
  }
